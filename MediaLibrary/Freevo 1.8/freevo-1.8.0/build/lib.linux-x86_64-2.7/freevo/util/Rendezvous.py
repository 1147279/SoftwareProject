# -*- coding: iso-8859-1 -*-
# -----------------------------------------------------------------------
# Rendezvous.py - Multicast DNS Service Discovery for Python, v0.10
# -----------------------------------------------------------------------
# $Id: Rendezvous.py 10562 2008-03-22 21:57:15Z duncan $
#
# Notes:
# Todo:
#
...
class DNSIncoming(object):
    ...
    def readName(self):
        ...
        while 1:
            len = ord(self.data[self.offset])
            self.offset += 1
            if len == 0:
                break
            t = len & 0xC0
            if t == 0x00:
                result = ''.join((result, self.readUTF(len) + '.'))
            elif t == 0xC0:
                if next < 0:
                    next = self.offset + 1
                self.offset = ((len & 0x3F) << 8) | ord(self.data[self.offset])
                if self.offset >= first:
                    raise "Bad domain name (circular) at " + str(hex(self.offset))
            else:
                raise "Bad domain name at " + str(hex(self.offset))
...
class Engine(threading.Thread):
    ...
    A reader needs a read() method, which is called when the socket
...
class Listener(object):
    ...
    def handle_read(self):
        ...
        if msg.isQuery():
            ...
            # Always multicast responses anyway
            #
...
class Rendezvous(object):
    ...
    def addServiceListener(self, type, listener):
        ...
        self.browsers.append(ServiceBrowser(type, listener))
    ...
    def unregisterService(self, info):
        ...
        try:
            self.services.remove(info.name.lower())
        except:
            pass
        now = currentTimeMillis()
        nextTime = now
